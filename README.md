...
**NOTE:** Do not modify the position of the reference crosses (Reference crosses layer) in the template (see [Fig. 1]( https://github.com/Mirijamz/MiGIS/blob/main/Manual_figures/Preprocessing_templates/Inkscape_reference.png)), they match the georeference points in [MiGIS_georef.points]( https://github.com/Mirijamz/MiGIS/blob/main/Preprocessing_templates/MiGIS_georef.points). If a new Inkscape project was created, it is required to create a top layer with reference points.
...
Figure 3: Georeferenced and stacked thin section mulit-band raster (left) vs. the clipped version (right), the applied clipping layer is in the center.
